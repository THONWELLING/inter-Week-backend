...
export default class PixService {
  ...
  async pay(key: string, user: Partial<User>) {
    ...
    if(keyDecoded.userId === user.id) {
      throw new AppError("It is not possible to receive the PIX from the same user", 401)
    }
    ...
    if(payingUser?.wallet && payingUser.wallet < Number(keyDecoded.value)) {
      throw new AppError('there is not enough balance to make the payment', 401)
    }

    if(!requestingUser || !payingUser) {
    throw new AppError('We did not find the transaction customers, please generate a new key', 401)
    }
    ...
    return {mag: 'Payment made successfully'}
  }
